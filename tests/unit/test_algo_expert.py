...
def test_algo_expert_unsupported_exchange():
    with pytest.raises(ValueError, match="Unsupported exchange: noname"):
        AlgoExpert(exchange="noname", api_key="", api_secret="", base_url="", instrument="", contract="", mode="", market_type="", contract_type="")

@patch('algoexpert.exchanges.binance.BinanceAdapter')
def test_algo_expert_initializes_binance_adapter(mock_adapter):
    expert = AlgoExpert(exchange="binance", api_key="key", api_secret="secret", base_url="url", instrument="inst", contract="cont", mode="mode", market_type="futures", contract_type="coin_m")
...
def test_algo_expert_initializes_bybit_adapter(mock_adapter):
    expert = AlgoExpert(exchange="bybit", api_key="key", api_secret="secret", base_url="url", instrument="inst", contract="cont", mode="mode", market_type="futures", contract_type="coin_m")
    mock_adapter.assert_called_once_with(
        api_key="key", 
        api_secret="secret", 
        base_url="url", 
...
@pytest.mark.parametrize("method_name, args, kwargs", [
    ...
    ("on_transaction", (4,), {}),
    ("on_book", (5,), {}),
    # TODO: Fix async test environment to properly test 'run' method
    # ("run", (), {}), 
])
@patch('algoexpert.exchanges.binance.BinanceAdapter')
